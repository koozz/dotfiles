...
(
	...
	if [[ -e /System ]]; then
		...
		if [[ -x "$(command -v rsvg-convert)" ]]; then
			...
			for file in $(git ls-files); do
				if [[ "${file}" =~ ^Pictures/zig-3840x2160.svg$ ]]; then
					if [[ ! -f "${HOME}/${file}.png" ]]; then
						rsvg-convert "${file}" >"${HOME}/${file}.png"
					fi
					osascript -e "tell application \"System Events\" to tell every desktop to set picture to \"${HOME}/${file}.png\" as POSIX file"
				fi
			done
		fi
	fi
	...
	for package in actionlint ast-grep bash-language-server biome deno diagnostic-languageserver fish-lsp gh-actions-language-server gopls graphql-language-service-cli helm-ls jq-lsp lua-language-server marksman mdsf regols ruff rust-analyzer shellcheck stylua superhtml trivy typescript-language-server vale-ls yaml-language-server zls; do
