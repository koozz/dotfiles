...
(
	...
	for package in actionlint ast-grep bash-language-server biome deno diagnostic-languageserver fish-lsp gh-actions-language-server gopls graphql-language-service-cli helm-ls jq-lsp lua-language-server marksman regols ruff rust-analyzer shellcheck stylua superhtml trivy typescript-language-server vale-ls yaml-language-server zls; do
		if [[ ! -d "${HOME}/.local/share/nvim/mason/packages/${package}" ]]; then
			printf "Installing %s with Mason\n" "${package}"
